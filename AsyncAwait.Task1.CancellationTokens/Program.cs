...
{
    private static Task<long> _task;
    ...
    {
        if (_task?.Status is TaskStatus.Running or TaskStatus.WaitingForActivation)
        {
            _cts.Cancel();
        }

        _cts = new CancellationTokenSource();
        _task = Task.Run(() => Calculator.Calculate(n, _cts.Token), _cts.Token);
